...
function App() {
  ...
  const [url, setUrl] = useState(
    "https://pokemonbackend-aldo.onrender.com/pokemon"
  );
  useEffect(() => {
    const fetchPokemon = () => {
  ...
  return (
    <div className="App">
      <Routes>
        ...
        <Route
          path="/pokemon"
          element={<Pokemon pokemon={pokemon} url={url} />}
        />
        <Route
          path="/pokemon/:id"
          element={<PokeDetails pokemon={pokemon} />}
        />
        <Route path="/pokemon/:name" element={<Pokemon pokemon={pokemon} />} />
        <Route
          path="/PlayGround/:id/fight"
          element={<PlayGround pokemon={pokemon} />}
        />
      </Routes>
    </div>
  );
